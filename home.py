...
analysis_tdf = st.file_uploader(label='analysis.tdf', type=['.tdf'])
frame_id_high = st.number_input(label='Frame id high', value=0)
frame_id_low = st.number_input(label='Frame id low', value=1)
...
if st.button('Run'):
    ...
    with sqlite_connect(analysis_tdf) as conn:
        ...
        st.write ("Average abs(time excess) = {0:.2f} %".format(100 * np.mean(np.abs(timediffs - exp_frame_time)) / exp_frame_time))
